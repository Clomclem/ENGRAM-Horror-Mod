package horror.blueice129.entity.ai.pathing;
...
public class PlayerLandPathNodeMaker extends PlayerPathNodeMaker {
    ...
    @Override
    public void init(ChunkCache cachedWorld, Blueice129Entity entity) {
    ...
    private boolean isBlocked(PathNode node) {
        ...
        Vec3d vec3d = new Vec3d(
                node.x - this.entity.getX() + box.getXLength() / 2.0,
                node.y - this.entity.getY() + box.getYLength() / 2.0,
                node.z - this.entity.getZ() + box.getZLength() / 2.0
        );
    ...
    protected PathNode getPathNode(int x, int y, int z, int maxYStep, double prevFeetY, Direction direction, PathNodeType nodeType) {
        ...
        if (d - prevFeetY > this.getStepHeight()) {
            ...
            double e = this.entity.getWidth() / 2.0;
            if (f >= 0.0F) {
            ...
            if (pathNodeType != PathNodeType.WALKABLE && (!this.isAmphibious() || pathNodeType != PathNodeType.WATER)) {
                if ((pathNode == null || pathNode.penalty < 0.0F)
                    ...
                    if (pathNode != null && (pathNode.type == PathNodeType.OPEN || pathNode.type == PathNodeType.WALKABLE) && this.entity.getWidth() < 1.0F) {
                        double g = x - direction.getOffsetX() + 0.5;
                        double h = z - direction.getOffsetZ() + 0.5;
                        Box box = new Box(
                                g - e,
                                this.getFeetY(mutable.set(g, (double)(y + 1), h)) + 0.001,
                                h - e,
                                g + e,
                                this.entity.getHeight() + this.getFeetY(mutable.set((double)pathNode.x, (double)pathNode.y, (double)pathNode.z)) - 0.002,
                                h + e
                        );
                        if (this.checkBoxCollision(box)) {
                            pathNode = null;
    ...
    @Override
    public PathNodeType getNodeType(BlockView world, int x, int y, int z, Blueice129Entity mob) {
    ...
    protected PathNodeType getNodeType(Blueice129Entity entity, BlockPos pos) {
        return this.getNodeType(entity, pos.getX(), pos.getY(), pos.getZ());
    }

    protected PathNodeType getNodeType(Blueice129Entity entity, int x, int y, int z) {
