...
import horror.blueice129.entity.ai.pathing.PlayerPathNodeMaker;
...
public class PlayerMoveControl implements Control {
    ...
    protected PlayerMoveControl.State state = PlayerMoveControl.State.WAIT;

    public PlayerMoveControl(Blueice129Entity entity) {
        this.entity = entity;
    }

    public boolean isMoving() {
        return this.state == PlayerMoveControl.State.MOVE_TO;
    }
    ...
    public void moveTo(double x, double y, double z, double speed) {
        ...
        this.targetZ = z;
        this.speed = speed;
        if (this.state != PlayerMoveControl.State.JUMPING) {
            this.state = PlayerMoveControl.State.MOVE_TO;
        }
    }

    public void strafeTo(float forward, float sideways) {
        this.state = PlayerMoveControl.State.STRAFE;
    ...
    public void tick() {
        if (this.state == PlayerMoveControl.State.STRAFE) {
            ...
            this.state = PlayerMoveControl.State.WAIT;
        } else if (this.state == PlayerMoveControl.State.MOVE_TO) {
            this.state = PlayerMoveControl.State.WAIT;
            ...
            if (o > this.entity.getStepHeight() && d * d + e * e < Math.max(1.0F, this.entity.getWidth())
                ...
                this.state = PlayerMoveControl.State.JUMPING;
            }
        } else if (this.state == PlayerMoveControl.State.JUMPING) {
            this.entity.setMovementSpeed((float)(this.speed * this.entity.getAttributeValue(EntityAttributes.GENERIC_MOVEMENT_SPEED)));
    ...
    private boolean isPosWalkable(float x, float z) {
        Blueice129Navigation entityNavigation = this.entity.getNavigation();
        if (entityNavigation != null) {
            PlayerPathNodeMaker pathNodeMaker = entityNavigation.getNodeMaker();
            if (pathNodeMaker != null
                    && pathNodeMaker.getDefaultNodeType(
                    this.entity.getWorld(), MathHelper.floor(this.entity.getX() + x), this.entity.getBlockY(), MathHelper.floor(this.entity.getZ() + z)
            )
